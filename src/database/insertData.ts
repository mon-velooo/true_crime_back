 ...
 */    await client.query('TRUNCATE TABLE "crime", "location_description", "district", "location_type", "age_group", "gender", "status", "law_category", "person" CASCADE');
 ...
 */      return res.rows[0].id;  // Si la valeur existe, retourne l'ID
  ...
  async function insertCrime(data:any) {
    ...
    let endTime = data.cmplnt_to_tm === '(null)' ? null : (data.cmplnt_to_tm ? data.cmplnt_to_tm : null);  // Si endTime est '(null)', on utilise `null`
    /* 
    console.log("-----------")
    ...
    const locationDescriptionId = await getOrInsert('location_description', 'description', data.loc_of_occur_desc);
    const districtId = await getOrInsert('district', 'name', data.boro_nm);
    const locationTypeId = await getOrInsert('location_type', 'label', data.prem_typ_desc);
    const suspectPersonId = await getOrInsertPerson(data.susp_age_group, data.susp_sex);
    const victimPersonId = await getOrInsertPerson(data.vic_age_group, data.vic_sex);
 ...
 */      return;  // Saute l'insertion si latitude ou longitude est manquant
    ...
    await client.query(
      `INSERT INTO "crime"("id", "start_date", "start_time", "end_date", "end_time", "latitude", "longitude", "description", "districtId", "locationDescriptionId", "locationTypeId", "suspectPersonId", "victimPersonId", "statusId", "lawCategoryId")
...
export async function fetchData() {
  try {
    ...
    const res = await axios.get('https://data.cityofnewyork.us/resource/5uac-w243.json?$limit=50000');
