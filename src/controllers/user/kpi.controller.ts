...
router.get("/getKpisByRange", async (req: Request, res: Response) => {
  try {
    ...
    const categories = {
      drugOffenses: ["CONTROLLED SUBSTANCE, SALE", "DRUG", "NARCOTICS"],
      propertyOffenses: [
        "LARCENY",
        "BURGLARY",
        "ROBBERY",
    ...
    const response = {
      ...
      kpis: {
        ...
        MISDEMEANOR: totalMisdemeanor,
        drugOffenses: totalDrugOffenses,
        propertyOffenses: totalPropertyOffenses,
        personOffenses: totalPersonOffenses,
      },
    };
